class class_DeepL
{
  ...
  [String] funcTranslate(){
    If($this.source_lang -eq "AUTO"){
      $ret = Invoke-WebRequest -Body @{ auth_key="$($this.auth_key)"; text="$($this.target_text)"; target_lang="$($this.target_lang)" } https://api-free.deepl.com/v2/translate
    }Else{
      $ret = Invoke-WebRequest -Body @{ auth_key="$($this.auth_key)"; text="$($this.target_text)"; source_lang="$($this.source_lang)"; target_lang="$($this.target_lang)" } https://api-free.deepl.com/v2/translate
    }
